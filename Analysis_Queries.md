...
# 13. Calculate year-wise total revenue
```sql
...
# 14. Display the customer details whose order amount is maximum including his past orders
```sql
...
# 15. Display the total amount ordered by each customer from high to low
```sql
SELECT c.*, SUM(totalamount) AS total_order_amount 
FROM customer c
JOIN orders o ON o.customerid = c.id
...
# 16. List the current and previous order dates for each customer
```sql
...
# 17. Find out the top 3 suppliers in terms of revenue generated by their products
```sql
...
# 21. Display all the product details with the ordered quantity size as 1
```sql
SELECT p.* 
FROM product p 
JOIN orderitem oi ON oi.productid = p.id 
...
# 22. Display the companies that supply products whose cost is above 100
```sql
SELECT DISTINCT s.* 
FROM supplier s 
JOIN product p ON p.SupplierId = s.id
...
#23. Display the customer list who belongs to the same city and country arranged country-wise
```sql
...
# 24. Calculate the total amount saved in each order and rank by savings
```sql
